...
final class SessionUpdateTests: XCTestCase {
    ...
    func testSessionUpdateEventHasEventEnvelope() throws {
        let json = BedrockEvents.sessionUpdateEvent(replace: ["A": "B"])
        let data = json.data(using: .utf8)!
        let parsed = try JSONSerialization.jsonObject(with: data) as! [String: Any]

        // Must have top-level "event" key
        XCTAssertNotNil(parsed["event"], "session.update must be wrapped in event envelope")
        let event = parsed["event"] as! [String: Any]

        // Must have "sessionUpdate" inside event
        XCTAssertNotNil(event["sessionUpdate"], "event must contain sessionUpdate key")

        // Must NOT have top-level type/replace/session (they're inside the envelope)
